            ...
            if (tabControl1.SelectedIndex == 0)
            {
                ...
                if (!string.IsNullOrWhiteSpace(clientNameTextBox.Text))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(shopNameTextBox.Text))
                    {
                        ...
                        "where invalid = 0 and (type = 0 and company_name like '%" + clientName + "%' and shop_name like '%" + shopName + "%' and name like '%" + clientStaff + "%' " +
                        "and address like '%" + address + "%') order by company_kana, shop_name_kana ";        //住所部分一致検索　　全部記入
                                conn.Open();
                            ...
                            else
                            {
                                string sql2 = "select company_name, shop_name, name, address, code, company_kana, shop_name_kana from client_m " +
                        "where invalid = 0 and (type = 0 and company_name like '%" + clientName + "%' and shop_name like '%" + shopName + "%' and name like '%" + clientStaff + "%')" +
                        "order by company_kana, shop_name_kana"; //住所のみ無記入
                                conn.Open();
                        ...
                        "order by company_kana, shop_name_kana";
                                conn.Open();
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(clientStaffNameTextBox.Text))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(addressTextBox.Text))
                            {
                                ...
                                string sql2 = "select company_name, shop_name, name, address, code, company_kana, shop_name_kana from client_m " +
                        "where invalid = 0 and (type = 0 and company_name like '%" + clientName + "%' and name like '%" + clientStaff + "%' and address like '%" + address + "%') " +
                        "order by company_kana, shop_name_kana";        //住所部分一致検索
                                conn.Open();
                        ...
                        "order by company_kana, shop_name_kana";          //住所部分一致検索
                            ...
                            {
                                string sql = "select company_name, shop_name, name, address, code, company_kana, shop_name_kana from client_m " +
                        "where invalid = 0 and (type = 0 and company_name like '%" + clientName + "%') " +
                        "order by company_kana, shop_name_kana";
                                conn.Open();
                ...
                else
                {
                    if (!string.IsNullOrWhiteSpace(shopNameTextBox.Text))
                    {
                        ...
                        #region "担当者名未記入"
                        {
                            ...
                            {
                                string sql = "select company_name, shop_name, name, address, code, company_kana, shop_name_kana from client_m " +
                            "where invalid = 0 and (type = 0 and shop_name like '%" + shopName + "%') " +
                            "order by company_kana, shop_name_kana";
                                conn.Open();
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(addressTextBox.Text))
                            {
                                ...
                                string sql2 = "select company_name, shop_name, name, address, code, company_kana, shop_name_kana from client_m " +
                                    "where invalid = 0 and (type = 0 and  address like '%" + address + "%') " +
                                    "order by company_kana, shop_name_kana";          //住所部分一致検索
                                conn.Open();
                            ...
                            {
                                MessageBox.Show("検索条件を入力して下さい。", "検索エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                return;
                            }
                            #endregion
                        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (radioButton1.Checked == true)//古物商許可証あり
                        {
                            check = 0;
                            string sql2 = "select name, address, antique_license, code, name_kana, address_kana from client_m " +
                                "where invalid = 0 and (type = 1 and  antique_license != '' )" +
                                "order by name_kana, address_kana"; //住所部分一致検索
                            conn.Open();
