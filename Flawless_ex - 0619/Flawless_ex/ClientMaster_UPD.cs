...
namespace Flawless_ex
{
    public partial class ClientMaster_UPD : Form
    {
        ...
        DataTable dt = new DataTable();// 選択したデータ
        int type;
        string name;
        string address;
        int staff_code;
        public ClientMaster_UPD(MasterMaintenanceMenu master,int type, string name, string address, int staff_code)
        {
            ...
            //this.shopname = shopname; // 店舗名
            this.name = name;　//担当者名または氏名
            this.address = address; // 住所
            this.type = type;  //  法人・個人
        }
        #region "下書き"
        private void ClientMaster_UPD_Load(object sender, EventArgs e)  // こっちには流れません。
        {
            NpgsqlConnection conn = new NpgsqlConnection();
            ...
            //　法人
            string sql_str1 = "select * from client_m_corporate where address = '" + address + "' and account_name = '" + name + "';";
            conn.Open();
            adapter1 = new NpgsqlDataAdapter(sql_str1, conn);
            adapter1.Fill(dt1);
            row1 = dt1.Rows[0];
            string RegistrationDate1;
            RegistrationDate1 = row1["registration_date"].ToString();
            ...
            DataRow row2;
            string sql_str2 = "select * from client_m_individual where type = 0;";

            conn.Open();

           
            adapter2 = new NpgsqlDataAdapter(sql_str2, conn);
            NpgsqlCommand cmd1 = new NpgsqlCommand(sql_str1, conn);
            NpgsqlDataReader reader1 = cmd1.ExecuteReader();
            NpgsqlCommand cmd2 = new NpgsqlCommand(sql_str2, conn);
            ...
            RegistrationDate2 = row2["registration_date"].ToString();
        ...
        private void ClientMaster_UPD_Load_1(object sender, EventArgs e)　//　こっちに流れます。
        {
            NpgsqlConnection conn = new NpgsqlConnection();
            NpgsqlDataAdapter adapter;
